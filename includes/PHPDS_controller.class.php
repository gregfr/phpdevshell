...
class PHPDS_controller extends PHPDS_dependant
{
	...
	public function runAJAX()
	{
		...
		if (isset($_SERVER["HTTP_X_REMOTE_CALL"])) {
			...
			if (method_exists($this, $f)) {
				...
				if (!empty($this->configuration['use_reflection'])) {
				...
				throw new PHPDS_remoteCallException('Ajax call for an unknown method "'.$f.'"');
			}
		} else {
     ...
     * - false in case of error (the framework then displays an error)
     * - true if if has handled the output (the framework doesn't output anything more)
     * - a string which is considered as the output to display
     * - an associative array ('output','result') with both output to display and result to propagate
     *
     * @see http://www.phpdevshell.org/content/return-values-controllers
	 *
	 * @version 1.2
	 * @since 3.0.5
	 * @author greg <greg@phpdevshell.org>
     ...
     * @date 20130305 (1.2) (greg) added support for controller's result propagation
	 *
		...
		if (false !== $encoded_data) {
			...
			if (is_null($raw_data)) { // deal with it the usual way (normal template)
				...
				if (false === $raw_data) { //  we consider it's an error
                    ...
                    $core->data = isset($raw_data['output']) ? $raw_data['output'] : '';
                    return isset($raw_data['result']) ? $raw_data['result'] : true;
				} else {
