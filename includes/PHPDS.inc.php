...
class PHPDS
{
     ...
     *
     * @date 20170119 (1.0.2) (greg) added a test to prevent session_regenerate_id() from sending an exception
     * @date 20150405 (1.0.1) (greg) added support for $configuration['cookie_lifetime'] ; minor refactoring
     * @date 20090427
     * @version	1.0.2
     *
     * @return PHPDS the current instance
    ...
    protected function configSession()
    {
        ...
        if (!empty($configuration['cookie_lifetime'])) {
            session_set_cookie_params($configuration['cookie_lifetime']);
        }

        if (empty($configuration['absolute_url'])) {
        ...
        try {
            if ($this->embedded) {
                ...
                if (!empty($this->configuration['absolute_url']))
                        $this->configuration['session_name'] = md5($configuration['absolute_url']);

                if (!empty($this->configuration['session_name']))
                        session_name(md5($configuration['session_name']));
                if (!empty($this->configuration['session_path']))
                        session_save_path($configuration['session_path']);

                session_start();
            }

            // Make sure we dont keep our session longer then it should be.
            if (!empty($configuration['session_life'])) {
...
//        $this->PHPDS_core()->loadDefaultPluginLanguage(); /////////////////////
     ...
     * @return PHPDS_array the configuration array
     *
...
class PHPDS_dependant
{
    ...
    /**
     * Holds a array of dependent extended object to appear as parent.
     * @var $parent array of object|string
     */
    ...
    /** @var array An array of values, to be use at the discretion of the object */
    protected $flags;
     ...
     * @version 1.1
     *
     * @date 1.1 (20170121) (greg) converted to parent list
     *
     * @param string $name
    ...
    public function __call($name, $arguments)
    {
        foreach ($this->parents as $parent) {
            ...
            if ((gettype($parent) == 'object') && method_exists($parent, $name)) {
                return call_user_func_array(array($parent, $name), $arguments);
            }
        }
        throw new Exception("No parent have a \"$name()\" method, maybe you're trying to call a protected/private method.");
    }

    /**
     * Add the given class (as string) or object as a parent
     *
     * @version 1.0
     *
     * @date 20170121 (1.0) (greg) Added
     *
    ...
    /**
     * Call a method (an "event") to each parents
     *
     * Each callee receives the result from the previous one (allows chaining)
     *
     * @param string $event The event name (corresponds to the method name of the parent)
     * @param mixed|null $params Parameters to send (optional)
     * @param bool|null $stack Should be return an array of results ($stack = true) or just the last one ($stack = false, default)
     * @param mixed|null $result First result to send to the first parent
     * @param null $progress Option progress parameter
     *
     * @return array|mixed
     */
    public function inform($event, $params = null, $stack = false, $result = null, $progress = null)
    {
        if ($stack !== false) {
            $stack = array();
        }
        foreach ($this->parents as $parent) {
            ...
            if ((gettype($parent) == 'object') && method_exists($parent, $event)) {
                $result = call_user_func(array($parent, $event), $params, $result);
                if (is_array($stack)) {
                    $stack[] = $result;
                }
            }
        }
        return is_array($stack) ? $stack : $result;
    }
     ...
     * @return null|mixed
     */
    public function flag($key, $value = null)
    {
        if (is_null($value)) {
 ...
 *
 * If a class is registered with its name prefixed with # it's auto-initialized (i.e. it's factored as a singleton
 *      after all classes are registered)
 *
 * If a class is factored with its name prefixed with & it's factored as a singleton (i.e. all subsequent factoring
...
class PHPDS_classFactory extends PHPDS_dependant
{
     ...
     *
     * A special item with index "#" contains auto-initialized classes
     *
     * @var array
     */
     ...
     *
     * If an alias starts with !, it's forced even if another class is already declared for this alias. This allows
     * late-binding declarations. By default, only the first binding is remembered, to give overrides in config files
     * higher precedence over database registry.
     *
     * @version 2.2
     *
     * @since 3.1.2
     ...
     * @date 20170119 (2.1) (greg) $classAlias can be a coma-separated list of aliases
     * @date 20170121 (2.2à (greg) added support for forced binding ("!")
     *
     * @param string $className the name of the PHP class to register
     * @param string $classAlias an alternate name for this class (can be a coma-separated list)
    ...
    public function registerClass($className, $classAlias, $pluginFolder, $fileName = null)
    {
        ...
        // first the class is added as itself
        if (empty($this->PluginClasses[$className])) {
            $this->PluginClasses[$className] = &$classData;
        }

        // and optionaly with its alias(es)
        $aliases = explode(',', $classAlias);
        foreach($aliases as $alias) {
            $forced = substr($alias, 0, 1) == '!';
            if ($forced) {
                $alias = substr($alias, 1);
            }
            if (!empty($alias) &&
                ($forced || empty($this->PluginClasses[$alias]))
            ) {
                $this->PluginClasses[$alias] = &$classData;
            }
            if (empty($this->PluginClasses['§'][$alias])) {
                $this->PluginClasses['§'][$alias] = array($classData);
            } else {
                $this->PluginClasses['§'][$alias][] = $classData;
            }
        }
    ...
    public function loadRegistry()
    {
        ...
        // if we don't have a registry in cache, we build it from the database and cache it
        ...
        if ($db->cacheEmpty('PluginClasses')) {
            ...
            if (! empty($pluginR)) {
                foreach ($pluginR as $p) {
                    ...
                    // the special symbol @ allows to specify a file where to find the class
        ...
        // once the registry is loaded, it's time to instantiate classes declared as auto-intialized
    ...
    /**
     * Find the actual classname, in case the provided name is an alias
     *
     * Does not actually check if the class exists: if it's not an alias, the provided input is
     * returned (whatever the field name)
     * (this is to allow support of un-registered classes)
     *
     ...
     * @return string|array
     */
    public function unAlias($classname, $field = null)
    {
        ...
        if (!empty($this->PluginClasses[$classname])) {
            $class = $this->PluginClasses[$classname];
        } else {
            return $classname;
        }

        if (!empty($field)) {
            return empty($class[$field]) ? '' : $class[$field];
        } else {
            return $class;
        }
    }
     ...
     */
    public function aliasList($aliasname)
    {
        return empty($this->PluginClasses['§'][$aliasname]) ? array() : $this->PluginClasses['§'][$aliasname];

        /*$result = array();

        foreach($this->PluginClasses as $index => $class) {
            if (!empty($class['class_name']) && ($class['class_name'] != $index) && !empty($class['alias']) && ($class['alias'] == $aliasname)) {
                $result[] = $class;
            }
        }
    ...
    public function factorClass($classname, $params = null, $dependancy = null)
    {
        ...
        try {
            ...
            $classname = $this->unAlias($classname, 'class_name');
    ...
    public function decorate(PHPDS_dependant $object, $classname)
    {
        ...
        foreach ($decorators as $decorator) {
            $decorator_class = $decorator['class_name'];
            $decorator = $this->factorClass($decorator_class, null, $object);
            $object->addParent($decorator);
        }
    }
