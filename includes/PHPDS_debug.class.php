...
class PHPDS_debug extends PHPDS_dependant
{
    ...
    /* @var boolean is the data to be sent, anyway */
    protected $enabled = false;

    /* @var integer level of verbosity */
    protected $level = PHPDS_debug::LOG;

    /* @var string to which semantic domain this instance is related (may be empty) */
    protected $domain;

    /* @var PHPDS_errorHandler a back link to the ErrorHandler to carry the messages */
     ...
     * @version 1.1.1
     *
     * @param string $domain the semantic domain of the debug object to match (or not)
     * the filters are set in configuration)
     */
    public function construct($domain = null)
    {
        $this->conduits = $this->errorHandler;

        // $domain will decide if it's enabled or not
     ...
     *
     * The domain is the semantic field of the data sent to this instance ; it's used to filter which
     * data will be actually sent
     * Note: the object will enable/disabled itself based on the domain debug configuration: the domain
     * MUST be in the array to be active.
     *
     * @version 1.1.1
     * @author greg
     *
     * @date 20161107 (1.1.1) (greg) domains where not properly fetched from global configuration
     * @date 20110216 (1.1) (greg) domain filters can now be regex
     *
