 ...
 * @author Jason Schoeman, Contact: titan [at] phpdevshell [dot] org.
 *
 * @date 20151204 (1.0.1) (greg) fixed two if's blocking input 'menu_name' and 'file_name'
 *
 */
        ...
        if (! empty($log_array) && $this->configuration['system_logging'] == true) {
            ...
            foreach ($log_array as $logged_data) {
                ...
                if (empty($logged_data['file_name'])) {
                    if (!empty($navigation[$this->configuration['m']]['menu_link'])) {
                ...
                if (empty($logged_data['menu_name'])) {
                    if (!empty($navigation[$this->configuration['m']]['menu_name'])) {
