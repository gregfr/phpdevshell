...
class PHPDS_taggerListQuery extends PHPDS_query
{
	...
	public function extraBuild($parameters = null)
	{
		if (!empty($parameters['object'])) {
            $this->addWhere("tagObject = '%(object)s'");
        }
		if (!empty($parameters['name'])) {
            $this->addWhere("tagName = '%(name)s'");
        }
		if (!empty($parameters['target'])) {
            $this->addWhere("tagTarget = '%(target)s'");
        }
		if (!empty($parameters['value'])) {
            $this->addWhere("tagValue = '%(value)s'");
        }
        if (!empty($parameters['id'])) {
            $this->addWhere("tagID = '%(id)s'");
        }
...
class PHPDS_taggerUntagQuery extends PHPDS_taggerListQuery
{
    protected $sql = "
		DELETE FROM
			_db_core_tags"; // WHERE tagTarget = '%s', tagObject = '%s', tagName = '%s', tagValue = '%s' ";
    ...
    public function checkParameters(&$parameters = null)
    {
        return (!empty($parameters['object']) && !empty($parameters['name']))
        || !empty($parameters['id']);
    }
